...
:term:`exception views <exception view>`. In general this is fine, but certain
view derivers may wish to avoid doing certain things when handling exceptions.
For example, the ``csrf_view`` and ``secured_view`` built-in view derivers will
not perform security checks on exception views unless explicitly told to do so.

In most cases, you can check for ``info.exception_only`` on the
:class:`pyramid.interfaces.IViewDeriverInfo` object. However, there is a
special case in which ``info.exception_only`` is ``False`` and
``info.options.get('exception')`` is not ``None`` where a view was registered
to handle both exceptions and a :term:`context` which is also an exception
type.
